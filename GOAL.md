...
Project precursor acquisition is fail-closed. S48 MOI official news may retain a `PROJECT_PRECURSOR_CANDIDATE` only when the record has a project marker, one of the four target sectors, and explicit forward-action evidence, while open Tender notices are excluded from precursor classification. A retained candidate is **not** a lifecycle event until human review explicitly promotes it. Promotion must preserve `canonical_items.created_at` as SignalForge's detection time; issuer publication date remains separate historical evidence and must never replace detection time. Project-to-procurement linkage remains explicit/reviewed; fuzzy linking is prohibited.
...
## Signal Quality / Actionability v1 production closure — 2026-09-11

PR #155 adds a deterministic **read-model quality score** for SignalForge business opportunities without changing the existing qualification/priority decision policy. The model is explicitly orthogonal to `priority_band`: priority answers **whether/how urgently to interrupt the user**, while Signal Quality answers **how complete and business-actionable the evidence is**. `QUALIFICATION_POLICY_VERSION` remains `1`; the new `SIGNAL_QUALITY_MODEL_VERSION` is `1`. The 100-point score uses issuer `15`, scope/quantity `20`, time `20`, location `10`, next action `10`, official evidence `10`, strategic relevance `10`, and urgency `5`; bands are `VERY_HIGH>=85 / HIGH>=70 / MEDIUM>=55 / REVIEW>=40 / LOW<40`. Quantity credit requires a numeric quantity plus a recognized unit/lot/set token; missing evidence is exposed as explicit gaps rather than inferred.

Fresh read-only production-copy calibration over the 15 current opportunities produced **VERY_HIGH=7 / HIGH=6 / MEDIUM=1 / REVIEW=1 / LOW=0; average=78.3** while leaving the old priority distribution unchanged at `HIGH=3 / MEDIUM=10 / REVIEW=2`. Representative results: Industry `89-93 / VERY_HIGH`; MOFA `86 / VERY_HIGH`; Energy `81 / HIGH`; Railways/IWT `71-78 / HIGH`; `mte:1605 = 59 / MEDIUM` while still `priority=REVIEW`; `doms:12735 = 40 / REVIEW`. This demonstrates the intended distinction: MTE is a real time-bound commercial Signal with incomplete lot/location/participation detail, whereas DOMS has materially weaker actionability because its timing and next action remain unknown. Production-copy DB SHA `00ab308c2381f130c90527166b3ed366ae3f1eba675650628ed16165d836f168` was identical before and after evaluation.

Targeted tests `41 passed`; full suite `295 passed`; PR #155 Actions run `34564204071` PASS. Exact production runtime is `b9fc1b3feaaa73ad9bbacade2dd5ad967a7cd234`; archive SHA256 `8a6e9ab34d2de44d4ada5417ce55f6b5e16c6070377e98be2f659d84f202a5fd`; rollback `cb847afc59e71ef89ba3ee57a1ca1707792ec7e0`. Live post-deploy verification reproduces `15 opportunities / priority 3-10-2 / quality 7-6-1-1 / quality average 78.3`, with Energy `81/HIGH`, MOFA `86/VERY_HIGH`, MTE `59/MEDIUM`, DOMS `40/REVIEW`. `business-digest --no-network` exposes the same quality distribution; future Telegram Attention messages render `Signal质量: score/100 · band`. Existing delivery identity is unchanged, so deployment did not replay prior alerts: Telegram dry-run `pending_count=0`, DB stayed `210 canonical / 51 signals / 5 immediate receipts`, backlog `0`, 27/27 sources GREEN, and acquisition/immediate Alert/daily Digest timers all remain active. No source refresh, canonical mutation, signal creation, schema migration, delivery receipt mutation, or priority-policy change was performed by this slice. Evidence: `docs/verification/SIGNAL-QUALITY-ACTIONABILITY-V1-PRODUCTION-CLOSURE-2026-09-11.md`.

## MTE reviewed image enrichment v1 production closure — 2026-09-11

PR #157 adds a fail-closed, source-scoped **read-model-only** reviewed enrichment for exact event `mte:1605`. It corrects two prior manual-review errors against the official MTE image: event time `09:30 -> 08:30`, and quantity interpretation `6 categories/types -> approximately 6,243 tons of teak/hardwood logs and sawn timber`. The official image SHA is `6a6c2452cf8ae18bf4985c3bd77f63bbd77ec710ad5b37ae3db51409854348e1`; the corrected reading was independently cross-checked with local Burmese/English OCR against that exact SHA. OCR remains supporting evidence, not an automatic canonical source.

The overlay applies only when S32/canonical key/item kind/reference/action date/article URL identity matches, fills only missing read-model fields, and never overwrites canonical/HTML facts. Auditor now re-fetches the reviewed official image and raises RED on SHA mismatch. The enriched production view keeps `priority=REVIEW / trust=B` while Signal Quality rises to `86 / VERY_HIGH`, demonstrating that evidence completeness and interrupt priority remain orthogonal. No bid deadline is inferred; the event remains `AUCTION_NOTICE / SELLER_OPEN_TENDER_SALE / BUY_FROM_ISSUER`.

Targeted enrichment/opportunity/Telegram tests `27 passed`; full suite `299 passed`; PR #157 Actions run `34620976113` PASS. PR #157 squash-merged as `6a1e8ea4a6b7df5f628ea428d959de50e0dc3f5c`; exact deployment archive SHA256 `c010edab2413fc57da4941bc0eecb778236fd166627c2e9e7204a6e44b4dc6b9` matched locally and on Bangkok; rollback target `b9fc1b3feaaa73ad9bbacade2dd5ad967a7cd234`. Live Bangkok acceptance returned the corrected `2026-09-15 08:30`, `~6,243 tons`, expected location, `Signal Quality 86/VERY_HIGH`, Auditor `PASS / findings=0 / reviewed MTE SHA match=1 / 27 sources non-green=0`, and Telegram dry-run `pending_count=0`, proving no duplicate replay. SignalForge Provider OCR authorization remains unchanged/disabled. Evidence: `docs/verification/MTE-REVIEWED-IMAGE-ENRICHMENT-V1-PRODUCTION-CLOSURE-2026-09-11.md`.

## Telegram Burmese → Chinese production closure — 2026-09-12

Presentation-only Myanmar/Burmese → Simplified-Chinese Telegram translation is production accepted. SignalForge PR #160 merged at `73e406c3b8be4d8a458e5c55a1765e5bf26c318a`; CI verify PASS (`34682781222` / `103524377014`), local full suite `315 passed`. Bangkok now runs that SHA with rollback `281bdcd4886ad51ab02262b11f87fec9e188d05c`; release archive SHA256 `53e28f694b50886beca28d77e2b6288b35ac1c2b1dfb845faa80d1275907cb3f`. Mac OAuth translation runtime is hardened at Browser Plane merge `3fee64c3fe7450bf1ed291cbbea19cf85b6485e0`; Mac full suite `72 passed`, doctor `READY`.

Default translation provider order is `mac_oauth_llm -> microsoft -> google -> original`. The Mac path uses the existing outbound pull SSH transport with a separate durable translation queue and exact `translation-claim/submit/fail/status-v1` verbs. Browser PIC remains C0–C3 only; no inbound Mac listener or generic shell authority was added. Codex translation turns are ephemeral/read-only with shell, browser, computer, apps and plugins disabled at CLI level.

Live BKK→Mac→OAuth→BKK smoke `10541b96-a973-4fce-8616-c9bb37de6a6c` succeeded with result `["公开招标 15.9.2026", "仰光"]`, protected date preserved, result SHA `af93b74c1a88084e7fffd41d100fc3e122941939f61d1c164c1621f8dfc639ee`; provider heartbeat was `ready=true`. A production Telegram renderer smoke (without Telegram API send) showed translated Chinese fields plus the machine-translation disclosure while preserving `6,243` and `TEST-6/2026`. Immediate and digest dry-runs both PASS with no pending send. Translation remains presentation-only and fail-open to original Myanmar text; canonical facts, Signal semantics, deadline/reference identity, dedup keys and Browser Plane capabilities are unchanged. See `docs/verification/TELEGRAM-BURMESE-TRANSLATION-MAC-OAUTH-PRODUCTION-CLOSURE-2026-09-12.md`.

## Source Yield / Signal Funnel audit v2 — 2026-09-12

S16/YCDC transient outage is closed with **no code/routing change**: eight consecutive `urlopen timeout` failures occurred from 08:10–08:55 UTC, followed by successful 5-item runs at 09:05 and 09:35. Direct BKK probes then returned HTTP 200 in ~0.3–0.7s; S16 and global SignalForge health returned to GREEN/PASS, recovery backlog=0, and S16 cumulative success is 216/224 (~96.4%). Browser fallback, timeout inflation and retry changes are explicitly not authorized.

Live funnel at `2026-09-12T09:56:08Z`: **27 active / 27 GREEN → 210 active canonical → 53 raw signals → 32 effective signals after 21 audited noise rows → 15 current opportunities (8 HIGH / 5 MEDIUM / 2 REVIEW) → 11 Telegram alerts**. S38+S21 contribute 10/15 current opportunities and 7/11 TG alerts; S13+S38+S21+S20 contribute 25/32 effective signals. S39 Energy and S30 MOFA are the two current ICT/Telecom opportunities, both HIGH.

The earlier 2026-09-10 audit's early-promotion gate is now triggered by S21: 4 effective signals, 4 current HIGH opportunities, 4 TG alerts, GREEN health in ~8.7 days. Scorecard v2 therefore promotes **S21 `STRATEGIC_WATCH → CORE` analytically only**. S22 remains Strategic Watch and S32 remains Observation. No runtime role, source priority, polling, parser, qualification, delivery or topology changes. Full audit: `docs/verification/SOURCE-YIELD-SIGNAL-FUNNEL-AUDIT-V2-2026-09-12.md`.

Source Yield / Signal Funnel v2 production closure: PR #162 CI PASS (`34687272715` / `103536343350`), merged/deployed release `4e95655be793bd2a22507d3209a76fe3bc87c7f6`, rollback `73e406c3b8be4d8a458e5c55a1765e5bf26c318a`, archive SHA256 `7f9d4b3215c2ea5d90f4704b3bc27121de65fd75c94aeb0f22514c7b3373aaeb`. Live scorecard v2 confirms 27/27 sources GREEN; S21 is CORE with 4 effective signals / 4 current HIGH opportunities / 4 TG alerts; S16 remains Strategic Watch and GREEN with consecutive_failures=0; global PASS/GREEN/backlog0. All production timers and Mac OAuth translation remained healthy.


## Construction + Telecom source expansion production closure — 2026-09-13

User-directed expansion was completed **without adding a Business Fit layer**. PR #164 adds three bounded issuer-original sources: S43 Yangon Region Ministry of Construction tenders, S44 ATOM 5G network technology announcements via the official media JSON API, and S45 MPT network/fiber/4G technology news. S23 national Ministry of Construction remains deferred because Bangkok strict TLS still rejects the issuer's expired certificate; TLS verification is not bypassed. Telecom strategic-intelligence records are `REGULATORY_NOTICE`, not procurement Tenders.

PR #164 verify run `34729187724` / job `103648717733` PASS; final local suite `320 passed`; merge/deployed release `14f0301a4dcceaaee9aa2451384c17d36ed1a7bb`; archive SHA256 `f72f8ceb735a02f939330d0b250147d81e27c7c05e3136a9c96f865d6de04bd6`; rollback `4e95655be793bd2a22507d3209a76fe3bc87c7f6`. Formal production baselines were S43 `2 canonical / 0 signals`, S44 `1 / 0`, S45 `3 / 0`. Production is now **30 active / 30 GREEN -> 216 active canonical (218 DB total) -> 53 raw signals -> 32 effective signals -> 15 current opportunities (8 HIGH / 5 MEDIUM / 2 REVIEW; 14 OPEN + 1 UNKNOWN) -> 11 cumulative Telegram alerts**. DB quick check is `ok`, recovery backlog `0`, immediate Telegram pending `0`, translation `ready=true`, and all three timers are enabled/active. The six new baseline rows therefore increased coverage while leaving Signal and opportunity counts unchanged. Evidence: `docs/verification/SOURCE-EXPANSION-CONSTRUCTION-TELECOM-2026-09-12.md`.


## PTD policy + strategic digest production closure — 2026-09-13

Telecom strategic coverage was extended without adding a Business Fit layer. `S46 PTD Spectrum and Telecom Policy` is now an `ACTIVE_SELECTIVE` Bangkok Direct-HTTP source over the official stable PTD policy table. It records bounded spectrum/5G/2600MHz, digital master-plan, Internet Exchange, numbering and connectivity policy events as `REGULATORY_NOTICE / STRATEGIC_INTELLIGENCE`, never as Tender opportunities. The daily Telegram Business Digest now renders up to four recent strategic-intelligence Signals with source, NEW/UPDATED state, strategic kind, publication date, title and official HTTPS link; immediate opportunity-alert policy is unchanged.

PR #166 CI run `34729932374` / job `103650719930` PASS; final suite `323 passed`; production runtime `f75767db4c30cc2a12f0fd9f1e166910082e72db`; archive SHA256 `777c676e3fa5a6550fc96fb0c36657ed6c7d84703e4c588d1c6178839a5d3861`; rollback `14f0301a4dcceaaee9aa2451384c17d36ed1a7bb`. Formal S46 production baseline is `9 canonical / 0 Signals`. Production is now **31 active / 31 GREEN -> 225 active canonical (227 DB total) -> 53 raw / 32 effective Signals -> 15 current opportunities (8 HIGH / 5 MEDIUM / 2 REVIEW; 14 OPEN + 1 UNKNOWN) -> 11 cumulative automatic Telegram alerts**. DB quick check is `ok`, immediate Telegram pending is `0`, translation is ready, and all three timers are enabled/active. Evidence: `docs/verification/PTD-POLICY-STRATEGIC-DIGEST-2026-09-13.md`.

## S23 National Ministry of Construction READY_TLS_BLOCKED closure — 2026-09-14

PR #172 completes the national MOC implementation without weakening strict TLS. The new listing-complete `moc_tender` parser uses the issuer `/letter-download/<uuid>` identity and preserves official title, region, date-only `End Date`, download URL and PDF metadata. A real saved issuer-page replay produced exactly the two reviewed current September tenders (2026-09-16 and 2026-09-23). S23 is fully specified in the registry but remains `enabled=false / READY_TLS_BLOCKED_ISSUER_CERT_EXPIRED`; a generic fail-closed Registry gate rejects an enabled source unless its `enable_only_after_gate_pass` gate is `PASS`. `deploy/check-moc-activation-gate.sh` performs strict HTTPS only and contains no certificate bypass.

PR #172 Actions run `34771313418` / job `103761309219` PASS; targeted tests `8 passed`, full suite `333 passed`; exact production runtime `b46f12b98d6a765c1a4a19e9c3c72dd96f764fb2`; archive SHA256 `45579ea4b6e4e61f7f361fe4826fdecb7e65f1b28f4e4727aadb3de8c6f6fe62`; rollback `b8bcf6d2d41d7899cdc21397b8ec029b624f8b84`. Post-deploy isolation is `31 active / S23 enabled=false / S23 scheduler_runs=0 / canonical=0 / signals=0`; all three timers remain active. Fresh Mac and Bangkok activation-gate checks both still fail with `curl 60 / certificate has expired`, so activation is not authorized. Global health immediately after deployment was DEGRADED/RED because S21 Myanma Railways had 33 fetch timeouts; that is unrelated to S23. Evidence: `docs/verification/S23-MOC-READY-TLS-BLOCKED-PRODUCTION-CLOSURE-2026-09-14.md`.
