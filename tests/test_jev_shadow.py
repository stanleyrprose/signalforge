...
def test_shadow_main_briefing_uses_calibrated_atomic_gate() -> None:
    assert DEFAULT_THRESHOLD == 0.60
    assert shadow_main_briefing(
        commercial_opportunity=0.95,
        sector=0.60,
        stage="open_opportunity",
    )
    assert not shadow_main_briefing(
        commercial_opportunity=0.95,
        sector=0.58,
        stage="open_opportunity",
    )
...
def test_shadow_report_is_read_only_comparison_with_deterministic_authority() -> None:
    tracked = [
        _row(
            source_id="SXX",
    ...
    assert result["authority"] == "DETERMINISTIC_ONLY"
    assert result["production_effect"] == "NONE"
    assert result["scope"]["false_negative_coverage_outside_current_opportunities"] == "NOT_PROVEN"
    ...
    assert [row["decision"] for row in result["rows"]] == [
        "AGREE_INCLUDE",
        "JEV_ONLY",
        "DETERMINISTIC_ONLY",
        "AGREE_EXCLUDE",
...
def test_shadow_report_requires_explicit_optional_credential_only_without_injected_evaluator(
...
def test_shadow_report_validates_limit_before_external_call() -> None:
    with pytest.raises(ValueError, match="limit"):
        jev_shadow_report(limit=0, evaluator=lambda _state: {})


def test_cli_exposes_shadow_command_without_changing_authority(capsys: pytest.CaptureFixture[str]) -> None:
    report = {
        "status": "SHADOW_ONLY",
    ...
    assert '\"authority\": \"DETERMINISTIC_ONLY\"' in capsys.readouterr().out
