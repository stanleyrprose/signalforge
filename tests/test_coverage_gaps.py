...
def test_reviewed_coverage_gaps_are_read_only_and_bounded() -> None:
    gaps = reviewed_coverage_gaps(now=datetime(2026, 9, 14, tzinfo=UTC))
    assert [item["deadline"] for item in gaps] == ["2026-09-16", "2026-09-23", "2026-09-29"]
    assert [item["source_id"] for item in gaps] == ["S23", "S23", "S13"]
...
def test_reviewed_coverage_gaps_expire_by_deadline_date() -> None:
    after_first = reviewed_coverage_gaps(now=datetime(2026, 9, 17, tzinfo=UTC))
    assert [item["deadline"] for item in after_first] == ["2026-09-23", "2026-09-29"]
    after_moc = reviewed_coverage_gaps(now=datetime(2026, 9, 24, tzinfo=UTC))
    assert [item["deadline"] for item in after_moc] == ["2026-09-29"]
    after_all = reviewed_coverage_gaps(now=datetime(2026, 9, 30, tzinfo=UTC))
    assert after_all == []
